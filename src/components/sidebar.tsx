...
const navegacao: NavItem[] = [
  ...
  { href: "/financeiro", label: "Financeiro", icon: Receipt, visivel: (p) => p.isAdmin || p.acessarFinanceiro },
  ...
  { href: "/notas-fiscais", label: "Notas Fiscais", icon: ClipboardList, visivel: (p) => p.isAdmin || p.acessarNotasFiscais },
  ...
  return (
    <>
      ...
      <button
        ...
        style={{ top: "var(--header-btn-top)" }}
        className={cn("fixed left-4 z-50 lg:hidden p-2 rounded-md bg-[#1a1208] text-[#B89968] shadow-md", aberto && "hidden")}
      >
        <Menu size={20} />
      </button>
